...
for BAMSF in bams1 bams2 bams3 bams4 bams5; do
	...
	time snakemake -p -s ~/work/chipseq-smk-pipeline/Snakefile all --use-conda --cores all  --directory $(pwd) --config fastq_dir=$(pwd) start_with_bams=True genome=hg38 lanceotron=True --rerun-incomplete --rerun-triggers mtime;
