T=$'\t'
echo "Modification${T}TruePeaks${T}Tool${T}Fdr${T}Peaks${T}Sensitivity${T}Specificity" > report.tsv
...
echo "SPAN gap 5"
for M in H3K27ac H3K27me3 H3K36me3 H3K4me1 H3K4me3; do echo $M; TRUTH=$(find /mnt/stripe/shpynov/2021_noise2/chr15/ -name "$M*"); echo $TRUTH; TP=$(cat $TRUTH | wc -l); echo "True peaks $TP"; for F in $(find /mnt/stripe/shpynov/2021_noise2/span/ -name "$M*_5.peak"); do echo $F; PEAKS=$(cat $F | wc -l); FDR=$(echo $F | sed -E 's/.*200_|_5.peak//g'); SENSITIVITY=$(bedtools intersect -a $TRUTH -b $F -wa -u | wc -l); SPECIFICITY=$(bedtools intersect -a $F -b $TRUTH -wa -u | wc -l); echo "Fdr $FDR Peaks $PEAKS Sensitivity $SENSITIVITY Specificity $SPECIFICITY"; echo "$M$T$TP${T}SPAN-5$T$FDR$T$PEAKS$T$SENSITIVITY$T$SPECIFICITY" >> report.tsv;  done;  echo ""; done

echo "SPAN gap 0"
for M in H3K27ac H3K27me3 H3K36me3 H3K4me1 H3K4me3; do echo $M; TRUTH=$(find /mnt/stripe/shpynov/2021_noise2/chr15/ -name "$M*"); echo $TRUTH; TP=$(cat $TRUTH | wc -l); echo "True peaks $TP"; for F in $(find /mnt/stripe/shpynov/2021_noise2/span/ -name "$M*_0.peak"); do echo $F; PEAKS=$(cat $F | wc -l); FDR=$(echo $F | sed -E 's/.*200_|_0.peak//g'); SENSITIVITY=$(bedtools intersect -a $TRUTH -b $F -wa -u | wc -l); SPECIFICITY=$(bedtools intersect -a $F -b $TRUTH -wa -u | wc -l); echo "Fdr $FDR Peaks $PEAKS Sensitivity $SENSITIVITY Specificity $SPECIFICITY"; echo "$M$T$TP${T}SPAN-0$T$FDR$T$PEAKS$T$SENSITIVITY$T$SPECIFICITY" >> report.tsv;  done;  echo ""; done
